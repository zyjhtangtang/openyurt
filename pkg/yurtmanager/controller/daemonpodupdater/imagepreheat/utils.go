...
func GetPodNextHashVersion(pod *corev1.Pod) string {
	for _, cond := range pod.Status.Conditions {
		if cond.Type == config.PodNeedUpgrade {
			return strings.TrimPrefix(cond.Message, config.VersionPrefix)
		}
	}
	return ""
}
...
func ExpectedPodImageReadyStatus(pod *corev1.Pod, expectCondition corev1.ConditionStatus) bool {
	for _, cond := range pod.Status.Conditions {
		if cond.Type == config.PodImageReady && cond.Status == expectCondition && strings.TrimPrefix(cond.Message, config.VersionPrefix) == GetPodNextHashVersion(pod) {
			return true
		}
	}
	return false
