...
func isUpgradeStatus(pod *corev1.Pod) bool {
	for _, cond := range pod.Status.Conditions {
		if cond.Type == config.PodNeedUpgrade && cond.Status == corev1.ConditionTrue {
			return true
		}
	}
	return false
...
func getImagePullJobName(pod *corev1.Pod) string {
	return config.ImagePullJobNamePrefix + pod.Name + "-" + strings.TrimPrefix(GetPodNextHashVersion(pod), config.VersionPrefix)
}
...
func (r *ReconcileImagePull) updatePodImageReady(pod *corev1.Pod, job *batchv1.Job) error {
	if job.Status.Succeeded > 0 {
		mesg := config.VersionPrefix + GetPodNextHashVersion(pod)
		return r.patchPodImageStatus(pod, corev1.ConditionTrue, config.PullImageSuccess, mesg)
	}
	if job.Status.Failed > 0 {
		mesg := fmt.Sprintf("pull image job %s failed", job.Name)
		return r.patchPodImageStatus(pod, corev1.ConditionFalse, config.PullImageFail, mesg)
	}
	return nil
}
