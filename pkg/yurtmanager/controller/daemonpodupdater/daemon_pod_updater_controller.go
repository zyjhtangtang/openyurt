...
func (r *ReconcileDaemonpodupdater) Reconcile(_ context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	v, ok := instance.Annotations[config.UpdateAnnotation]
	if !ok {
	...
	case strings.ToLower(config.OTAUpdate):
	...
	case strings.ToLower(config.AutoUpdate), strings.ToLower(config.AdvancedRollingUpdate):
...
func (r *ReconcileDaemonpodupdater) syncPodsOnNodes(ds *appsv1.DaemonSet, podsToDelete []string) error {
	...
	if deleteDiff > config.BurstReplicas {
		deleteDiff = config.BurstReplicas
	}
...
func (r *ReconcileDaemonpodupdater) maxUnavailableCounts(ds *appsv1.DaemonSet, nodeToDaemonPods map[string][]*corev1.Pod) (int, error) {
	...
	v, ok := ds.Annotations[config.MaxUnavailableAnnotation]
	if !ok || v == "0" || v == "0%" {
		v = config.DefaultMaxUnavailable
	}
