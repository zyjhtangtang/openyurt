...
func (r *ReconcileDaemonpodupdater) SetPodUpgradeCondition(ds *appsv1.DaemonSet, pod *corev1.Pod, newHash string) error {
	...
	cond := &corev1.PodCondition{
		...
		Status:  status,
		Message: config.VersionPrefix + newHash,
	}
...
func checkPrerequisites(ds *appsv1.DaemonSet) bool {
	v, ok := ds.Annotations[config.UpdateAnnotation]
	if !ok || (!strings.EqualFold(v, config.AutoUpdate) && !strings.EqualFold(v, config.OTAUpdate) && !strings.EqualFold(v, config.AdvancedRollingUpdate)) {
		return false
	}
...
func GetPodUpgradeCondition(status corev1.PodStatus) *corev1.PodCondition {
	_, condition := podutil.GetPodCondition(&status, config.PodNeedUpgrade)
	return condition
}
