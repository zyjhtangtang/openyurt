...
func createPodWithWrongImageVersion(name, namespace, nodeName string) *corev1.Pod {
	...
	pod.Status.Conditions = append(pod.Status.Conditions, corev1.PodCondition{
		...
		Status:  corev1.ConditionTrue,
		Reason:  "Ready",
		Message: config.VersionPrefix + "wrong-version",
	})
	return pod
}
...
func TestGetPodImageReadyCondition(t *testing.T) {
	...
	assert.Equal(t, config.PodImageReady, cond.Type)
}

// Test ImagePullPod function
func TestImagePullPod(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ImagePullPod(clientset, tt.nodeName).ServeHTTP(rr, req)
